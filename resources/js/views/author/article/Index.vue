...
export default {
    ...
    methods: {
        ...
        showDeleteDialog(id){
            console.log(id);
            axios.get(`/api/author/article/${id}`, {
                headers: {
                    Authorization: `Bearer ${this.token}`
                }
            })
                .then(({ data }) => {
                    ...
                    this.deleteDataArticle.id = responseData.id;
                    this.deleteDataArticle.title = responseData.title;
        ...
        deleteArticle(id){
            axios.delete(`/api/author/article/${id}`, {
                headers: {
                    Authorization: `Bearer ${this.token}`
                }
            })
...
<template>
    <BaseLayout>
        <template #content>
                    ...
                    <template v-for="(item, index) in dataTemp" :key="index">
                        <tr v-show="checkView(index + 1)"
                            ...
                            <td class="flex px-4 py-4 space-x-2 text-right">
                                ...
                                <router-link :to="{ name: 'Edit Article Author Page', params: { slug: item.slug } }" target="_blank"
            ...
            <div v-show="isDeleteDialog" class="flex justify-center items-center absolute top-0 right-0 bottom-0 left-0">
                <div class="relative p-4 w-full max-w-md max-h-full">
                    <div class="relative bg-white rounded-lg shadow-sm">
                        <button @click="isDeleteDialog=false" type="button" class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center cursor-pointer">
                        ...
                        <div class="p-4 md:p-5 text-center">
                            ...
                            <h3 class="mb-5 text-lg font-normal text-gray-500">{{ deleteDataArticle.title }}</h3>
                            <button @click="deleteArticle(deleteDataArticle.id)" type="button" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center cursor-pointer">
                                Hapus
                            </button>
                            <button @click="isDeleteDialog=false" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 cursor-pointer">
                                Batal
                            </button>
                        </div>
                    </div>
